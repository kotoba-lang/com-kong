(ns kong.main
  "Kotodama WASM entrypoint for the Kong clean-room actor (L4) — Clojure port.
  ...
  facts (`kong.<Entity>/<field>`); `*store*` is the in-memory materialization
...
(def entity-specs
  [{:entity "Service"   :plural "services"   :id-prefix "kong_ser"
    :fields [:name :protocol :host :port :path :retries :enabled :connectTimeout] :required [:name]
    :coerce {:port :int :retries :int :connectTimeout :int :enabled :bool} :refs {}}
   {:entity "Route"     :plural "routes"     :id-prefix "kong_rou"
    :fields [:name :service :stripPath :preserveHost :priority :pathHandling :httpsRedirectStatusCode] :required [:name]
    :coerce {:stripPath :bool :preserveHost :bool :priority :int :httpsRedirectStatusCode :int} :refs {}}
   {:entity "Consumer"  :plural "consumers"  :id-prefix "kong_con"
    :fields [:username :customId] :required [:username]
    :coerce {} :refs {}}
   {:entity "Plugin"    :plural "plugins"    :id-prefix "kong_plu"
    :fields [:name :instanceName :service :route :consumer :enabled] :required [:name]
    :coerce {:enabled :bool} :refs {}}
   {:entity "Upstream"  :plural "upstreams" :id-prefix "kong_ups"
    :fields [:name :algorithm :hashOn :hashFallback :hashOnHeader] :required [:name]
    :coerce {} :refs {}}
   {:entity "Target"    :plural "targets"    :id-prefix "kong_tar"
    :fields [:upstream :target :weight] :required [:upstream]
    :coerce {:weight :int} :refs {}}])
...
(defn emit-facts
  "EAVT facts for one record: {\"kong.<Entity>/<field>\" v ...}. The datomic
...
(defn healthz [] [{:status "ok" :actor "kong-compat" :tier tier :entities entities} 200])
